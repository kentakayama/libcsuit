...
# +minimize libcsuit
ARG FROM_IMAGE=i386/ubuntu:latest
FROM ${FROM_IMAGE}
ARG FROM_IMAGE

RUN apt-get update
RUN DEBIAN_FRONTEND=noninteractive apt-get -y install curl git musl-tools make python3 python3-pip
ENV CC="musl-gcc -static -m32 -std=c99"
...
RUN pip3 install -r ./scripts/basic.requirements.txt
RUN CFLAGS="-Os -fdata-sections -ffunction-sections" make no_test -j`nproc`
RUN make install
